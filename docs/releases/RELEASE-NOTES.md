...
These preview installers are not distribution-signed or notarized. Linux compatibility depends on the build baseline (x64 Ubuntu 22.04, arm64 Ubuntu 24.04 in CI; local arm64 evaluation build Ubuntu 26.04). Match the architecture to your machine and check SHA256SUMS before installing.

Known limits: Windows and its own WSL2 did not discover each other in the test topology; direct-address transfers worked. Interrupted transfers are not resumable. Persistent trusted-device pairing is not implemented. The 60-transfer CLI matrix was supplemented by macOS↔Windows UI tests that transferred large and small files concurrently in both directions and verified their hashes. Direct-address CLI → macOS desktop was also validated with sender identity, progress and a 2 GiB SHA-256 match. Desktop → CLI and the direct-address case on Windows remain unverified.
