...
Use the same Apple team and suitable credentials as Markdraw. Do not commit these values or paste them into logs or chat. `TAURI_SIGNING_PRIVATE_KEY` is for updater signatures and is not a replacement for the Apple credentials; Dukto does not currently have an updater.
...
After Tauri builds the DMG, CI checks the bundled `Dukto.app` using `codesign --verify`, `xcrun stapler validate` and `spctl --assess`. Collection and checksums happen only after those checks succeed. The app's identifier remains `com.dukto.app`.
