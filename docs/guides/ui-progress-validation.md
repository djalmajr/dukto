...
Evidências locais: `.cache/integration/direct-cli/after-modal.png`, `after-progress.png`, `after-result.json` e `after.log`. A tentativa de observar o menu de outro host durante o progresso foi interrompida pela mudança de estado da UI; não é evidência de persistência desse menu. O Windows posteriormente repetiu CLI → desktop direto com identidade isolada e 1 GiB: modal `Integration CLI Windows@Alien`, card sem menu, barra verde com 43,6 MB/s e depois 72,4 MB/s em 731,1 MB / 1 GB. A aceitação foi feita pelo agente, sem intervenção manual observada. ACK de 1.073.741.824 bytes e SHA-256 `43b49d55240b3aebb5cdae098a7a33d9ecc4328797351e7c3512423f8a7a5250` coincidiram; ambas as fixtures foram removidas. Evidências na sessão Windows: `windows_direct_cli_ui_validation`. Desktop → CLI permanece sem teste nativo nesta rodada.
