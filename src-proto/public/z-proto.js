...
class ZProto extends HTMLElement {
	...
	connectedCallback() {
		// Adopt stylesheet into document (idempotent)
		...
		const extrasEl = this.querySelector("z-proto-window-extras");
		const contentEl = this.querySelector("z-proto-window-content");

		// Read initial attributes
		this.#state.width = Number(this.getAttribute("window-width")) || DEFAULT_WIDTH;
		this.#state.height = Number(this.getAttribute("window-height")) || DEFAULT_HEIGHT;
	...
	#render() {
		const presetOptions = [
			el("option", { value: "responsive" }, "Responsive"),
		...
		const dimGroup = el("div", { className: "zp-dim-group" }, [
			widthInput,
			el("span", { className: "zp-dim-sep" }, "\u00D7"),
			heightInput,
		]);

		const toolbar = el("div", { className: "zp-toolbar" }, [
			presetSelect,
			dimGroup,
			zoomSelect,
			figmaLink,
		...
		const header = el("div", { className: "zp-header", "data-ref": "header", hidden: true });

		// Traffic lights
		const trafficLights = el("div", { className: "zp-traffic-lights" }, [
			el("span", { className: "zp-tl zp-tl-red" }),
			el("span", { className: "zp-tl zp-tl-yellow" }),
			el("span", { className: "zp-tl zp-tl-green" }),
		]);

		const titleCenter = el("div", { className: "zp-titlebar-center", "data-ref": "title" });
		const titleExtras = el("div", { className: "zp-titlebar-extras", "data-ref": "extras" });

		const titlebar = el("div", { className: "zp-titlebar" }, [
			trafficLights,
			titleCenter,
			titleExtras,
		]);

		const contentInner = el("div", { className: "zp-content-inner", "data-ref": "content" });
		const contentScroll = el("div", { className: "zp-content-scroll" }, contentInner);
		const contentArea = el("div", { className: "zp-content" }, contentScroll);

		const windowEl = el("div", { className: "zp-window", "data-ref": "window" }, [
			titlebar,
			contentArea,
		]);
		...
		const handles = EDGES.map((edge) =>
			el("div", { className: `zp-handle zp-handle-${edge}`, "data-edge": edge }),
		);

		const frameWrapper = el("div", { className: "zp-frame-wrapper", "data-ref": "frame" }, [
			scaleWrapper,
			...handles,
		]);

		const viewportCenter = el("div", { className: "zp-viewport-center" }, frameWrapper);

		const stage = el("div", { className: "zp-stage" }, [toolbar, header, viewportCenter]);
...
class ZProtoWindowContent extends HTMLElement {}
...
if (!customElements.get("z-proto-window-extras")) {
	customElements.define("z-proto-window-extras", ZProtoWindowExtras);
}
if (!customElements.get("z-proto-window-content")) {
	customElements.define("z-proto-window-content", ZProtoWindowContent);
}
